...
let repeated (value: 'a) (len: int): 'a list = List.init len (fun _ -> value)
...
let score_all_cards (cards: card list): int =
    ...
    let accumulate (acc: int list) (winners: int): (int list*int) =
        ...
        let new_cards = repeated copies winners in
        (list_add rest new_cards, copies)
    in 
    let acc = repeated 1 (List.length cards) in
    let _, card_counts = List.fold_left_map accumulate acc scores in
    sum card_counts

let puzzle_input = read_file parse_card "04.txt"

let part_one (cards: card list) = List.map score_card cards |> sum
