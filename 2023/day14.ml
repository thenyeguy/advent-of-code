...
let part_one (input : char matrix) = input |> tilt North |> compute_load_north

(*
 * Part 2
 *)
let run_spin_cycle (dish : char matrix) : char Matrix.t =
  dish |> tilt North |> tilt West |> tilt South |> tilt East

let rec run_spin_cycles (cycles : int) (dish : char matrix) : char Matrix.t =
  if cycles > 0 then run_spin_cycles (cycles - 1) (run_spin_cycle dish)
  else dish

exception FoundCycle of cycle

let find_cycle (dish : char matrix) : cycle =
