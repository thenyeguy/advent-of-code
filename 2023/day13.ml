 ...
 * Part 1
 *)
let find_row_reflection ?(smudges = 0) (pattern : char matrix) : int option =
...
let find_col_reflection ?smudges (pattern : char matrix) : int option =
  pattern |> Matrix.transpose |> find_row_reflection ?smudges

let count_reflections ?smudges (patterns : char matrix list) : int =
...
let part_one (patterns : char matrix list) : int = count_reflections patterns

(*
 * Part 2
 *)
let part_two (patterns : char matrix list) : int =
